...
def decide_overlap_old(sliceA, sliceB, alpha=0.1):
    ...
    for m in overlap_to_check:
        print(m)
        pi, log = partial_pairwise_align_given_cost_matrix(sliceA, sliceB, M=M, alpha=alpha, m=m, armijo=False,
    ...
    for i in range(len(overlap_to_check) - 1, -1, -1):
        if m_to_edge_inconsistency_A[i - 1] < m_to_edge_inconsistency_A[i]:
            half_estimate_A = overlap_to_check[i]
            break
    for i in range(len(overlap_to_check) - 1, -1, -1):
        if m_to_edge_inconsistency_B[i - 1] < m_to_edge_inconsistency_B[i]:
            half_estimate_B = overlap_to_check[i]
            break

    return 2 * min(half_estimate_A, half_estimate_B)
...
def decide_overlap_plotting_old(sliceA, sliceB, alpha=0.1):
    overlap_to_check=[0.99, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5, 0.45, 0.4, 0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.05]
    # subset for common genes
    common_genes = intersect(sliceA.var.index, sliceB.var.index)
    sliceA = sliceA[:, common_genes]
